...
{
    Result<IReadOnlyList<ProcessedRow<Folder>>> GetAllItems(File pathToFolders);
}

public sealed class ExcelFolderRepository : IFolderRepository
{
    public Result<IReadOnlyList<ProcessedRow<Folder>>> GetAllItems(File pathToFolders)
    {
                ...
                for (int i = 1; i <= sheet.LastRowNum; i++)
                {
                        ...
                        row.GetCell(0).NumericCellValue != 0)
                    {
                        if (row.GetCell(1) != null && row.GetCell(1).StringCellValue.Trim() != string.Empty)
                        {
                            ...
                            processedRows.Add(new ProcessedRow<Folder>(i + 1, false, Folder.Create(builder.ToString(), Path.IsPathRooted)));
